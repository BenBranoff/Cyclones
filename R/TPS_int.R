TPS_int <- function(L,lines,centers,s_res=20000){
  if (L==length(unique(lines$date))){return(NULL)}
  lines$date <- as.POSIXct(lines$date,tz="GMT")
  ###  find the outer bounds of the storm
  outerext<- lines |>filter(quad=="track") |> summarise() |>st_buffer(max(lines$dist_m_mean[lines$quad=="ROCI"],na.rm=T),nQuadSegs=2)
