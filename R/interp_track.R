...
interp_track <- function(track,t_res){
  ...
  for (n in 1:(nrow(track))){
    ...
    if (n==nrow(track)){
      ...
      end_point <- track[n,] %>%
        ...
        st_cast("POINT")%>%
        mutate(ISO_TIME =t2)
      st_geometry(end_point)="geometry"
    }else{
    ...
    if (dt>t_res/60){
      ...
      dt <- as.numeric(difftime(int_times,lag(int_times),units="hours"))
      ...
      t  <- track[n,] %>%
        ...
        st_transform(st_crs(track))%>%
        st_cast("POINT")%>%
        mutate(ISO_TIME = int_times[2:(length(int_times)-1)])#track$ISO_TIME[n]+60*15*seq(1,length(dt)))
      st_geometry(t)="geometry"
  ...
  points <- rbind(points,newpoints,end_point)%>%
    arrange(ISO_TIME)
