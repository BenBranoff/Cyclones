...
make_extent <- function(tracks,mods,type="linestrings"){
  ...
  track <- lapply(1:nrow(tracks),function(L,trs,m){
    ...
    trs$LAT <- as.numeric(trs$LAT)
    trs$LON <- as.numeric(trs$LON)
    custCRSall <- paste0("+proj=lcc +lat_0=",round(mean(trs$LAT,na.rm=TRUE))," +lon_0=",round(mean(trs$LON,na.rm=TRUE))," +lat_1=",round((max(trs$LAT,na.rm=TRUE)-min(trs$LAT,na.rm=TRUE))/6),
                        " +lat_2=",round(5*(max(trs$LAT,na.rm=TRUE)-min(trs$LAT,na.rm=TRUE))/6)," +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs +type=crs")
    ...
    Cat <- as.numeric(tr$USA_SSHS)
    maxwind <- as.numeric(tr$USA_WIND)
    ...
    maxwinddist <- as.numeric(tr$USA_RMW)
    if (length(maxwinddist)==0){maxwinddist <- NA}
    minpress <- as.numeric(tr$USA_PRES)
    ...
    if (is.na(minpress)){
      minpress=minpresss$USA_PRES[minpresss$USA_SSHS==Cat&minpresss$MONTH==tr$MONTH&minpresss$BASIN==basin]
      if (length(minpress)==0){
        minpress=mean(minpresss$USA_PRES[minpresss$MONTH==tr$MONTH&minpresss$BASIN==basin],na.rm=TRUE)
      }
    }
    maxpress <- as.numeric(tr$USA_POCI)
    ...
    ROCId <- as.numeric(tr$USA_ROCI)
    if (length( ROCId)==0){ ROCId=NA}
    EYE <- as.numeric(tr$USA_EYE)
    if (length(EYE)==0){ EYE=NA}
    swathpoints <- c(NE34 = tr$USA_R34_NE,SE34 = tr$USA_R34_SE,NW34 = tr$USA_R34_NW,SW34 = tr$USA_R34_SW,
                     NE50 = tr$USA_R50_NE,SE50 = tr$USA_R50_SE,NW50 = tr$USA_R50_NW,SW50 = tr$USA_R50_SW,
                     NE64 = tr$USA_R64_NE,SE64 = tr$USA_R64_SE,NW64 = tr$USA_R64_NW,SW64 = tr$USA_R64_SW)
    ...
    if (sum(swathpoints,na.rm=T)==0){
      ...
      if (is.na(maxwinddist)){
        ...
        names(repl) <- suppressWarnings(gsub(paste(gsub("NE|SE|SW|NW","",names(repl)),collapse="|"),maxwind,names(repl)))
      }
    ...
    if (is.na(EYE)&Cat>0){
      ...
      if (eyerad<min(swathpoints[swathpoints>0])){
        ...
        linestrings <- rbind(linestrings,suppressWarnings(st_cast(eye,"LINESTRING")))
      }
    }
    if (is.na(ROCId)){
      if (Cat>0){
    ...
    linestrings <- bind_rows(linestrings,suppressWarnings(st_cast(ROCI,"LINESTRING")))
    ...
    swaths$basin <- basin; linestrings$basin <- basin
